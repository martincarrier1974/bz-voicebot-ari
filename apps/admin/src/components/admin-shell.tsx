  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#070B14] dark:text-slate-100">
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <aside className="hidden w-72 border-r border-slate-200 bg-white px-5 py-6 dark:border-white/10 dark:bg-[#070B14] lg:block">
...
export function Card({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5 dark:backdrop-blur">
      <p className="text-sm text-slate-500 dark:text-slate-300/70">{title}</p>
      <p className="mt-3 text-3xl font-semibold">{value}</p>
      {hint ? <p className="mt-2 text-xs text-slate-400 dark:text-slate-300/60">{hint}</p> : null}
    </div>
  );
}
...
export function Section({
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/5 dark:backdrop-blur">
      <div className="mb-4">
        <h3 className="text-lg font-semibold">{title}</h3>
        {description ? <p className="mt-1 text-sm text-slate-500 dark:text-slate-300/75">{description}</p> : null}
      </div>
      {children}
    </section>
  );
